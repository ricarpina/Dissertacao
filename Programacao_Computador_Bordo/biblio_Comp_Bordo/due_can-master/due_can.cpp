...
uint8_t CANRaw::verificar_RTR(uint8_t uc_index){

			uint32_t var_temp = m_pCan->CAN_MB[uc_index].CAN_MCR & CAN_MCR_MRTR;
			return var_temp == CAN_MCR_MRTR;
}
/**
...
bool CANRaw::sendFrame(CAN_FRAME& txFrame) 
{
	for (int i = 0; i < 8; i++) {
		...
		{//is this mailbox set up as a TX box?
			...
			{//is it also available (not sending anything?)
				...
				if(txFrame.rtr>1) txFrame.rtr=1;
				mailbox_set_rtr(i, txFrame.rtr);
				if(txFrame.rtr==0)
    ...
    tx_frame_buff[tx_buffer_tail].rtr = txFrame.rtr;
...
uint32_t CANRaw::mailbox_read(uint8_t uc_index, volatile CAN_FRAME *rxframe)
{
	...
	rxframe->rtr = verificar_RTR(uc_index);
...
void CANRaw::mailbox_set_rtr(uint8_t uc_index, uint32_t rtr) 
{
	uint32_t var_temp = CAN_MCR_MRTR;
	if(rtr==1)
	m_pCan->CAN_MB[uc_index].CAN_MCR |= var_temp;
	else if(rtr==0)
		m_pCan->CAN_MB[uc_index].CAN_MCR &=~var_temp;
...
uint32_t CANRaw::get_rx_buff(CAN_FRAME& buffer) {
	...
	buffer.rtr = rx_frame_buff[rx_buffer_tail].rtr;
	...
	if (m_pCan->CAN_MB[mb].CAN_MSR & CAN_MSR_MRDY) { //mailbox signals it is ready
		...
		case 3: //transmit
			...
			{ //if there is a frame in the queue to send
				...
				if( tx_frame_buff[tx_buffer_head].rtr>1)  tx_frame_buff[tx_buffer_head].rtr=1;
				mailbox_set_rtr(mb,  tx_frame_buff[tx_buffer_head].rtr);
				if(tx_frame_buff[tx_buffer_head].rtr==0)
